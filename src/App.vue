    ...
    <div style="height:48px;">
      <div class="search"
        ...
        <button class="btn-search"
          @click="getInfo"></button>
        <transition name="bounce">
          <input v-if="searchClicked"
            placeholder="城市名，拼音或汉字均可"
  ...
  export default {
    ...
    created: function () {
      if (localStorage.vue_weather_location) {
        this.location = localStorage.vue_weather_location || '成都';
      }
    },
    async mounted() {
      try {
      ...
      if (window.Notification && Notification.permission !== "denied") {
        Notification.requestPermission().then(() => {
          ...
          n.onclick = e => {
            ...
            initServer()
            // socket = new WebSocket('ws://10.0.0.14:8777');
          }
        })
      }
    },
